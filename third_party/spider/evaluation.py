...
PARTIAL_TYPES = ['select', 'select(no AGG)', 'where', 'where(no OP)', 'where(with value)', 'group(no Having)',
...
def eval_where(pred, label, pred_with_val, label_with_val):
    pred_conds = [unit for unit in pred['where'][::2]]
    pred_conds_with_val = [str(unit).lower() for unit in pred_with_val['where'][::2]]
    label_conds = [unit for unit in label['where'][::2]]
    label_conds_with_val = [str(unit).lower() for unit in label_with_val['where'][::2]]
    ...
    for unit in pred_conds_with_val:
        if unit in label_conds_with_val:
            cnt_with_val += 1
            label_conds_with_val.remove(unit)

    return label_total, pred_total, cnt, cnt_wo_agg, cnt_with_val
...
def eval_nested(pred, label, pred_with_val, label_with_val):
    label_total = 0
    pred_total = 0
    cnt = 0
    cnt_with_val = 0
    ...
    if pred is not None and label is not None:
        partial_scores = Evaluator.eval_partial_match(pred, label, pred_with_val, label_with_val)
        cnt += Evaluator.eval_exact_match(pred, label, {k:v for k,v in partial_scores.items() if 'with value' not in k})
        cnt_with_val += Evaluator.eval_exact_match(pred, label, partial_scores)
    return label_total, pred_total, cnt, cnt_with_val


def eval_IUEN(pred, label, pred_with_val, label_with_val):
    lt1, pt1, cnt1, cnt1_with_val = eval_nested(pred['intersect'], label['intersect'], pred_with_val['intersect'], label_with_val['intersect'])
    lt2, pt2, cnt2, cnt2_with_val = eval_nested(pred['except'], label['except'], pred_with_val['except'], label_with_val['except'])
    lt3, pt3, cnt3, cnt3_with_val = eval_nested(pred['union'], label['union'], pred_with_val['union'], label_with_val['union'])
    label_total = lt1 + lt2 + lt3
    pred_total = pt1 + pt2 + pt3
    cnt = cnt1 + cnt2 + cnt3
    cnt_with_val = cnt1_with_val + cnt2_with_val + cnt3_with_val
    return label_total, pred_total, cnt, cnt_with_val
...
class Evaluator:
    ...
    @classmethod
    def eval_partial_match(cls, pred, label, pred_with_val, label_with_val):
        res = {}
        ...
        label_total, pred_total, cnt, cnt_wo_agg, cnt_with_val = eval_where(pred, label, pred_with_val, label_with_val)
        acc, rec, f1 = get_scores(cnt, pred_total, label_total)
        res['where'] = {'acc': acc, 'rec': rec, 'f1': f1,'label_total':label_total,'pred_total':pred_total}
        acc, rec, f1 = get_scores(cnt_wo_agg, pred_total, label_total)
        res['where(no OP)'] = {'acc': acc, 'rec': rec, 'f1': f1,'label_total':label_total,'pred_total':pred_total}
        acc, rec, f1 = get_scores(cnt_with_val, pred_total, label_total)
        res['where(with value)'] = {'acc': acc, 'rec': rec, 'f1': f1,'label_total':label_total,'pred_total':pred_total}
        ...
        label_total, pred_total, cnt, cnt_with_val = eval_IUEN(pred, label, pred_with_val, label_with_val)
        acc, rec, f1 = get_scores(cnt, pred_total, label_total)
    ...
    def evaluate_one(self, db_name, gold, predicted):
        ...
        g_sql_with_val = copy.deepcopy(rebuild_sql_col(g_valid_col_units, g_sql, kmap))
        g_sql = rebuild_sql_val(g_sql)
        g_sql = rebuild_sql_col(g_valid_col_units, g_sql, kmap)
        
        try:
            p_valid_col_units = build_valid_col_units(p_sql['from']['table_units'], schema)
            p_sql_with_val = copy.deepcopy(rebuild_sql_col(g_valid_col_units, p_sql, kmap))
        except:
            p_sql = {
            "except": None,
            "from": {
        ...
        if self.etype in ["all", "exec"]:
            ...
            try:
                ...
                exec_score, is_empty, is_error = eval_exec_match(self.cached_db[db_name], predicted, gold, p_sql, g_sql)
        ...
        if self.etype in ["all", "match"]:
            partial_scores = self.eval_partial_match(p_sql, g_sql, p_sql_with_val, g_sql_with_val)
            exact_score = self.eval_exact_match(p_sql, g_sql, {k:v for k,v in partial_scores.items() if 'with value' not in k})
            exact_score_with_val = self.eval_exact_match(p_sql, g_sql, partial_scores)
    ...
    def finalize(self):
        ...
        for level in LEVELS:
            ...
            if self.etype in ["all", "exec"]:
                ...
                scores[level]['exec (non empty)'] = sum(scores[level]['exec (non empty)'])/len(scores[level]['exec (non empty)'])
                scores[level]['is_empty'] /= scores[level]['count']
                scores[level]['is_error'] /= scores[level]['count']

            if self.etype in ["all", "match"]:
                scores[level]['exact'] /= scores[level]['count']
                scores[level]['exact (with val)'] /= scores[level]['count']
                for type_ in PARTIAL_TYPES:
